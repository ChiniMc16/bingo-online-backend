...
app.post('/api/payments/webhook', async (req, res) => {
    ...
    try {
        ...
        const { Payment } = mercadopago.resources; // ✅ FIX
        const paymentsInstance = new Payment(client); // ✅ FIX
        const mpPaymentResponse = await paymentsInstance.get({ id: paymentId });
        ...
        if (!gameId || !userId) return res.status(200).send('OK');
